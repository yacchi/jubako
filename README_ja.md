...
Jubako は、単に「全部読み込んで 1 つのマップにまとめる」だけでは足りない用途を対象にしています。レイヤーを保持したまま、
解決済みの値がどこから来たかを追跡し、正しいレイヤーへ書き戻し、対応フォーマットではコメントや整形も保ちながら更新できます。

特に、設定を読むだけでなく編集も行う CLI やアプリケーション、ホットリロードが必要なもの、あるいは 1 つの論理設定を
複数の物理ストアにまたがって扱いたいケースに向いています。

複数の設定ソースをレイヤーとして扱い、優先度に従って最終的な設定を解決します。名前は重箱にちなんでいます。

[English README](README.md)

設計資料: [docs/design/README_ja.md](docs/design/README_ja.md) / [docs/design/README.md](docs/design/README.md)
...
- **レイヤーを前提にした設計** - 複数ソースを 1 つに潰さず、レイヤー単位の情報を保持
- **オリジン追跡** - 解決済みの値がどのレイヤーから来たかをパス単位で確認可能
- **レイヤー単位の書き戻し** - 特定レイヤーを更新し、変更のあるレイヤーだけを保存
- **フォーマット保持編集** - 対応する YAML / TOML / JSONC を、全体再生成ではなくパッチ適用で更新
- **型安全な解決結果** - 最終結果を `Store[T]` として型付きで取得
- **購読可能なホットリロード** - 監視対象の変更を読み直し、解決済み設定の変更を通知
- **論理層と物理層の協調** - メタデータファイルと外部シークレットストアのような複数バックエンドから、
  1 つの論理レイヤーを構成可能
...
- [coordinated-layer](examples/coordinated-layer/) - メタデータ + 外部シークレットストアを 1 つの論理レイヤーとして扱う例
...
多くの設定ライブラリは、ファイル・環境変数・フラグ・リモートソースなどから設定を読み込み、マージする用途に十分強力です。
Jubako が狙っているのは、それに加えて「読み込んだ後もレイヤーが見え、追跡でき、書き戻せる」問題領域です。

| 観点 | Jubako が重視する点 | 他の一般的なライブラリで重視されやすい点 |
|------|---------------------|------------------------------------------|
| 解決済み設定 | 最終結果と per-layer データの両方を保持 | マージ済みの最終結果を扱いやすくする |
| デバッグ性 | 解決済みの値に origin 情報を持たせる | 最終値へのアクセスを中心にすることが多い |
| 編集 | 対象レイヤーを指定して更新し、dirty な変更だけ保存 | 読み込みと struct への反映を主目的にすることが多い |
| ファイル保持 | 対応フォーマットではコメントやレイアウトを保って更新 | 再生成や全体書き換えの戦略が一般的 |
| 実行中の更新 | 解決済みストアに対して `Subscribe` と `Watch` を提供 | reload 支援の形はライブラリごとに異なる |
| パス操作 | JSON Pointer ベースで `GetAt`、`SetTo`、`DeleteFrom`、`Walk` を提供 | キー文字列や struct unmarshalling が中心になりやすい |
| 外部ストア協調 | 複数バックエンドから 1 つの論理レイヤーを再構成できる | 複数ストア間の協調はアプリ側実装に委ねられやすい |
| 拡張境界 | `Source`、`Document`、`Layer` を分離して差し替え可能 | 拡張ポイントはライブラリやエコシステムごとに異なる |

単純に設定を読み込んで struct に入れたいだけなら、既存の定番ライブラリが適している場面も多くあります。Jubako は、
provenance、フォーマット保持を伴う安全な書き戻し、あるいは複数ストアにまたがる論理設定の協調が必要になると、より強みが出ます。
