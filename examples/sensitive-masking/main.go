...
//
// Sensitivity rules:
// - `jubako:"sensitive"` marks a LEAF field as containing sensitive data
// - Only leaf fields (string, int, etc.) should have the sensitive tag
// - Container types (struct, map, slice) should NOT have the sensitive tag
// - Each sensitive field must be explicitly marked - there is NO inheritance
type AppConfig struct {
	// App is a normal (non-sensitive) struct
	App AppSettings `json:"app"`

	// Credentials contains sensitive fields - each is explicitly marked
...
type Credentials struct {
	// Each sensitive field must be explicitly marked
	APIKey    string `json:"api_key" jubako:"sensitive"`    // Sensitive (explicit)
	SecretKey string `json:"secret_key" jubako:"sensitive"` // Sensitive (explicit)

	// PublicKey is not marked sensitive
	PublicKey string `json:"public_key"` // NOT sensitive (no tag)
}
...
type DBConfig struct {
	...
	// Password is individually marked sensitive
...
func demonstrateMasking(ctx context.Context) {
	...
	fmt.Println("  - `jubako:\"sensitive\"` marks a LEAF field as sensitive")
	fmt.Println("  - Each sensitive field must be explicitly marked (NO inheritance)")
	fmt.Println("  - Only leaf fields (string, int, etc.) should have the sensitive tag")
	fmt.Println()
	...
	paths := []struct {
		...
		{"/credentials/api_key", "sensitive (explicit)"},
		{"/credentials/secret_key", "sensitive (explicit)"},
		{"/credentials/public_key", "normal field (no tag)"},
...
func demonstrateWriteProtection(ctx context.Context) {
	...
	fmt.Println("  - Normal fields can be written to any layer")
	fmt.Println("  - This prevents accidental exposure of sensitive data")
	fmt.Println()
	...
	fmt.Println("   Field: sensitive (explicit tag)")
	...
	// Case 2: Normal field -> Sensitive layer (OK - normal fields can go anywhere)
	fmt.Println()
	...
	// Case 5: Non-sensitive field in credentials -> Normal layer (OK, no sensitive tag)
	fmt.Println()
	fmt.Println("5. SetTo(\"project\", \"/credentials/public_key\", \"new-public-key\")")
	fmt.Println("   Field: normal (no sensitive tag)")
	fmt.Println("   Layer: normal")
